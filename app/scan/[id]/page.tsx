...
export default function ScanPage({ params }: { params: { id: string } }) {
  ...
  // For live part state updates after actions
  const [currentStation, setCurrentStation] = useState("");
  const [currentStatus, setCurrentStatus]   = useState("");

  // For work-order fallback — station picker
  ...
  async function handleAction(action: "started" | "completed" | "failed_qc" | "scrapped") {
    ...
    const station =
      data.type === "part" ? currentStation : selectedStation;
    if (!station) return;
                ...
                >
                  ...
                  <span
                    className={`text-sm flex-1 ${
                      isCurrent ? "font-bold text-blue-800" : "text-gray-500"
                    }`}
                  >
                    {s}
                  </span>
                  {isCurrent && !isTerminal && (
                    <span className="text-xs text-blue-400 font-medium">← here</span>
                  )}
                  {isFuture && (
                    <span className="text-xs text-gray-300">Step {i + 1}</span>
                  )}
                </div>
              );
            })}
  ...
  return (
    <Shell woNumber={wo.wo_number} partNumber={wo.part_number} customer={wo.customer_name} qty={wo.planned_qty}>
      {/* Operator name */}
      {!nameEntered && (
        <div className="px-4 py-3 border-b border-gray-100 bg-yellow-50">
          <p className="text-xs text-yellow-700 mb-1.5 font-medium">Enter your name (optional)</p>
      ...
      <div className="px-4 py-4">
        {confirmed ? (
          ...
          <p className="text-center text-sm text-gray-400 py-4">
            ↑ Select a station to record a scan
          </p>
        )}
      </div>
    </Shell>
